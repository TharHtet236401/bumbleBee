...
export const readLeaveRequestByClass = async (req, res, next) => {
  try {
    ...
    if (!classPermission) {
      return next(
        new Error(
          "You don't have permission to view the leave requests from other class"
        )
      );
    }
    ...
    for (let i = 0; i < reader.classes.length; i++) {
      for (let j = 0; j < reader.childern.length; j++) {
        let requests = await LeaveRequest.find({
          classId: reader.classes[i],
          studentId: reader.childern[j],
        })
          .populate("senderId", "userName")
          .populate("studentId", "name")
...
export const editLeaveRequest = async (req, res, next) => {
  try {
    ...
    if (!classPermission) {
      return next(
        new Error(
          "You don't have permission to edit the leave requests from other class"
        )
      );
    }
