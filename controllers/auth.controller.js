...
export const passwordReset = async (req, res) => {
    ...
    try {
        ...
        if (!email.trim() || !newPassword.trim()) {
            return fMsg(res, "Password reset failed", "Email and new password are required", 400);
        }
        const user = await User.findOne({ email });
...
export const changePassword = async (req, res) => {
    try {
        ...
        if (!email.trim() || !oldPassword.trim() || !newPassword.trim() || !confirmedNewPassword.trim()) {
            return fMsg(res, "Password change failed", "Email, old password, new password and confirmed new password are required", 400);
        }
        ...
        console.log(isMatch)
        if (!isMatch) {
            return fMsg(res, "Password change failed", "Invalid old password", 400);
        }

        if (newPassword !== confirmedNewPassword) {
            return fMsg(res, "Password change failed", "New password and confirmed new password do not match", 400);
        }
